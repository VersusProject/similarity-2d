 ...
 * 
 */
public class AdjustedRandIndexMeasureTest 
{
	@Test
	public void test()
	{
	...
	// unlabeled case: whole image (no labels)
		...
		try {
			...
			System.out.println( "AdjustedRandIndexMeasure Similarity value (unlabeled)\t" + result.getValue() + "\t" + fileName1 + "\t" + fileName2  );
	...
	// labeled case
		...
		try {
			...
			AdjustedRandIndexMeasure lm 		= new AdjustedRandIndexMeasure();
			SimilarityNumber[] lresult 	= lm.compare(ldesc1, ldesc2);
			System.out.print( "AdjustedRandIndexMeasure Similarity value (labeled)\t{");
	...
	{
		new AdjustedRandIndexMeasureTest().test();
	}
}
