...
public class DicePixelMeasureTest extends junit.framework.TestCase
{
	...
	// unlabeled case: whole image (no labels)	
		...
		try {
			...
			DicePixelMeasure m 		= new DicePixelMeasure();
				if ( m == null )
					throw new SWIndependenceException("failed to create DicePixelMeasure object for measure");				
			...
			String SEP = ",";
			String rString = fileName1 + SEP + fileName2 + SEP + "ImageObjectAdapter" + SEP + "ThreeDimensionalDoubleArrayFeature" + SEP + "DicePixelMeasure" + SEP + result.getValue() ;
	...
	public static SimilarityNumber[] labeled( String fileName1, String fileName2 ) throws Exception
	{
		...
		try {
			...
			DicePixelMeasure lm 		= new DicePixelMeasure();			
				if ( lm == null )
					throw new SWIndependenceException("failed to create DicePixelMeasure object for measure");	
...
////////////////	
	...
	{
		org.junit.runner.JUnitCore.runClasses( DicePixelMeasureTest.class );
	}
}