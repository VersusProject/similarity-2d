 ...
 * Version: 1.0
 *    Date: Fri Dec 23 20:02:52 EST 2011
 */
 ...
 * 
 */
public class InnerProductMeasureTest 
{
	@Test
	public void test()
	{
	...
	// grayscale
		...
		try {
			...
			System.out.println( "InnerProductMeasure Similarity value (Grayscale)\t" + result.getValue() + "\t" + fileName1 + "\t" + fileName2  );
	...
	// rgb
		...
		try {
			...
			InnerProductMeasure rm 		= new InnerProductMeasure();
			SimilarityNumber rresult 	= rm.compare(rdesc1, rdesc2);
			System.out.println( "InnerProductMeasure Similarity value (RGB)\t" + rresult.getValue() + "\t" + fileName1 + "\t" + fileName2  );
	...
	{
		new InnerProductMeasureTest().test();
	}
}
