 ...
 * Version: 1.0
 *    Date: Fri Dec 23 20:02:46 EST 2011
 */
 ...
 * 
 */
public class KulczynskiMeasureTest 
{
	@Test
	public void test()
	{
	...
	// grayscale
		...
		try {
			...
			System.out.println( "KulczynskiMeasure Similarity value (Grayscale)\t" + result.getValue() + "\t" + fileName1 + "\t" + fileName2  );
	...
	// rgb
		...
		try {
			...
			KulczynskiMeasure rm 		= new KulczynskiMeasure();
			SimilarityNumber rresult 	= rm.compare(rdesc1, rdesc2);
			System.out.println( "KulczynskiMeasure Similarity value (RGB)\t" + rresult.getValue() + "\t" + fileName1 + "\t" + fileName2  );
	...
	{
		new KulczynskiMeasureTest().test();
	}
}
