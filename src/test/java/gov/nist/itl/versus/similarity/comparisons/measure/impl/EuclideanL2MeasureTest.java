 ...
 * Version: 1.0
 *    Date: Fri Dec 23 20:02:44 EST 2011
 */
 ...
 * 
 */
public class EuclideanL2MeasureTest 
{
	@Test
	public void test()
	{
	...
	// grayscale
		...
		try {
			...
			System.out.println( "EuclideanL2Measure Similarity value (Grayscale)\t" + result.getValue() + "\t" + fileName1 + "\t" + fileName2  );
	...
	// rgb
		...
		try {
			...
			EuclideanL2Measure rm 		= new EuclideanL2Measure();
			SimilarityNumber rresult 	= rm.compare(rdesc1, rdesc2);
			System.out.println( "EuclideanL2Measure Similarity value (RGB)\t" + rresult.getValue() + "\t" + fileName1 + "\t" + fileName2  );
	...
	{
		new EuclideanL2MeasureTest().test();
	}
}
