 ...
 * Version: 1.0
 *    Date: Fri Dec 23 20:29:33 EST 2011
 */
 ...
 * 
 */
public class TotalErrorRateEvaluationMeasureTest 
{
	@Test
	public void test()
	{
	...
	// unlabeled case: whole image (no labels)
		...
		try {
			...
			System.out.println( "TotalErrorRateEvaluationMeasure Similarity value (unlabeled)\t" + result.getValue() + "\t" + fileName1 + "\t" + fileName2  );
	...
	// labeled case
		...
		try {
			...
			TotalErrorRateEvaluationMeasure lm 		= new TotalErrorRateEvaluationMeasure();
			SimilarityNumber[] lresult 	= lm.compare(ldesc1, ldesc2);
			System.out.print( "TotalErrorRateEvaluationMeasure Similarity value (labeled)\t{");
	...
	{
		new TotalErrorRateEvaluationMeasureTest().test();
	}
}
